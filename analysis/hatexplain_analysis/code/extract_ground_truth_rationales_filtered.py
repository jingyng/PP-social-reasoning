...
def extract_ground_truth_rationales_filtered():
    """Extract ground truth rationales for offensive and hate speech labels only"""
    # Load data
    ...
    print("Filtering for 'Offensive language' and 'Hate speech' labels only...")
    ...
    for post_id, sample_info in selected_samples.items():
        ...
        # Filter: only include offensive and hate speech
        if majority_label not in ['Offensive language', 'Hate speech']:
            continue
    ...
    print(f"Samples with offensive/hate labels: {filtered_count}")
    print(f"Filtering ratio: {filtered_count/total_processed:.1%}")

    return ground_truth_rationales


def save_ground_truth_rationales_filtered(ground_truth_rationales: Dict):
    """Save filtered ground truth rationales to file"""
    output_dir = Path("3_analysis/3_hatexplain_analysis")
    output_dir.mkdir(exist_ok=True)

    output_file = output_dir / "ground_truth_rationales_filtered.json"

    with open(output_file, 'w', encoding='utf-8') as f:
        json.dump(ground_truth_rationales, f, indent=2, ensure_ascii=False)

    print(f"Saved filtered ground truth rationales to: {output_file}")

    # Also save as JSONL for easier processing
    output_jsonl = output_dir / "ground_truth_rationales_filtered.jsonl"
    with open(output_jsonl, 'w', encoding='utf-8') as f:
        for data in ground_truth_rationales.values():
            f.write(json.dumps(data, ensure_ascii=False) + '\n')

    print(f"Saved filtered ground truth rationales to: {output_jsonl}")


def main():
    print("Extracting filtered ground truth rationales (offensive + hate speech only)...")

    ground_truth_rationales = extract_ground_truth_rationales_filtered()
    ...
    print(f"\nFiltered dataset contains {len(ground_truth_rationales)} samples")
    print(f"Label distribution: {dict(label_counts)}")
    print(f"Rationale statistics: {rationale_stats}")

    # Save results
    save_ground_truth_rationales_filtered(ground_truth_rationales)

    print("\nFiltered ground truth rationale extraction completed!")
