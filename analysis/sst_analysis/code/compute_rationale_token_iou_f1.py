...
def load_ground_truth_rationales(group: str) -> Dict[str, List[int]]:
    ...
    gt_file = GT_DIR / f"{group}_processed.json"
    gt_rationales = {}
...
def load_model_persona_predictions(model_dir: Path) -> Dict[str, Dict[str, List[int]]]:
    ...
    for path in sorted(model_dir.glob("QID*.jsonl")):
        question_id = path.stem  # e.g., 'QID1001'
        ...
        with path.open("r", encoding="utf-8") as f:
            for line in f:
                ...
                # Extract persona code from persona_id (e.g., "QID1001___25_m_b" -> "25_m_b")
...
def load_baseline_predictions(model_dir: Path, model_name: str) -> Dict[str, List[int]]:
    ...
    possible_names = [
        ...
        f"baseline_{model_name.split('_')[0]}_sst.jsonl",  # e.g., mistral_sst.jsonl
        f"baseline_sst.jsonl"
    ]
...
def main():
    print("Computing SST Token-F1 and IOU-F1 performance...")
    ...
    baseline_per_run_csv = output_dir / "baseline_token_iou_f1_sst_per_run.csv"
    ...
    persona_per_run_csv = output_dir / "persona_token_iou_f1_sst_per_run.csv"
    ...
    baseline_averaged_csv = output_dir / "baseline_token_iou_f1_sst_averaged.csv"
    ...
    persona_averaged_csv = output_dir / "persona_token_iou_f1_sst_averaged.csv"
    ...
    print("\nSST Token-F1 and IOU-F1 computation completed!")
