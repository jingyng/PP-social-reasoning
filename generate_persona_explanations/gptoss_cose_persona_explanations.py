...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("../logs/gptoss_persona_cose.log", mode='a', encoding='utf-8')  # Log to logs folder
    ]
)
...
def _normalize_persona_id(pid: str) -> str:
    ...
    Converts patterns like "__25" or "____25" to "___25" to match stored IDs.
    """
    try:
...
parser = argparse.ArgumentParser(description="Generate GPT-OSS COSE persona explanations")
parser.add_argument(
    "--results-dir",
    default="../datasets/results_gpt_oss_120b_cose_r2_medium",
...
def _iter_persona_runs(question_id: str, personas: List[Dict[str, Any]], done_persona_ids: Set[str]):
    ...
    for p_idx, persona in enumerate(personas):
        ...
        persona_id = persona_id_from_parts(question_id, persona)
        ...
        yield p_idx, persona_text, canonical_persona_id, persona
...
for q_idx, question in enumerate(questions, start=1):
    ...
    with output_path.open("a", encoding="utf-8") as f_out:
        for _, persona_text, canonical_persona_id, persona in _iter_persona_runs(
