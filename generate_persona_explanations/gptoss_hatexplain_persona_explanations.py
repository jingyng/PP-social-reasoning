...
DEFAULT_RESULTS_DIR = "../datasets/results_gpt_oss_120b_500_r3_medium"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("../logs/gptoss_persona_prompting.log", mode='a', encoding='utf-8'),
    ],
)
...
def _join_with_commas_and(parts: List[str]) -> str:
...
def persona_description(persona: Dict[str, str]) -> str:
    ...
    loneliness = persona.get("loneliness")
    education = persona.get("education")
    religion = persona.get("religion")
    political = persona.get("political_view")

    core_bits: List[str] = []
    if age is not None:
        core_bits.append(f"{age}-year-old")
    if race:
        core_bits.append(str(race))
    if gender:
        core_bits.append(str(gender))

    clauses: List[str] = []
    if core_bits:
        core_phrase = " ".join(core_bits)
        needs_article = age is not None or (race and gender)
        if needs_article:
            clauses.append(f"is {_article_for(core_phrase)} {core_phrase}")
        else:
            clauses.append(f"is {core_phrase}")

    extras: List[str] = []
    if education:
        extras.append(f"has {education}")
    if religion:
        extras.append(f"is {_article_for(str(religion))} {religion}")
    if political:
        extras.append(f"has {political} political views")
    if loneliness:
        extras.append(f"is {loneliness}")

    clauses.extend(extras)

    description = _join_with_commas_and(clauses)
    return description or "has an unspecified background"
...
parser = argparse.ArgumentParser(description="Generate GPT-OSS HateXplain persona explanations, skipping existing entries.")
parser.add_argument(
    "--results-dir",
            ...
            for attempt in range(3):
                try:
                    response = client.chat.completions.create(
                        ...
                        temperature=1.0,
                        extra_body={"reasoning": {"effort": "medium", "enabled": True}},
                    )
